...
| externalSecrets | object | `{"oauth2":{"key":null},"service":{"key":"my.test.secret","refreshInterval":"15m"}}` | -----------------------------# |
...
| externalSecrets.service.key | string | `"my.test.secret"` | `Key` to AWS Secret Manager object where all sensitive application data should be stored. Each key in the Secret Manager Object should be named like your needed environment variable |
...
| gloo.ingress.scope | string | `"public"` | Signals Gloo which Gateway Proxy to use for deploying a Virtual Service into. Value must be one of `private` for intranet- or `public` for internet-deployment. |
