...
| externalSecrets | object | `{"oauth2":{"key":null},"service":{"key":null,"refreshInterval":null,"secretstoreName":null}}` | -----------------------------# |
...
| externalSecrets.service.refreshInterval | string | `nil` | `refreshInterval` set the sync time range from AWS Secret Manager. Default is set to 15m |
| externalSecrets.service.secretstoreName | string | `nil` | `secretstoreName` is the Name of the SecretStore inside the Release Namespace. If empty, default is set to `external-secret-store-NAMESPACE` |
