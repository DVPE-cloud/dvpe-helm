...
| gloo.authConfig.spec.configs.authExtensionPlugin.config.enableAccessTokenForwarding | bool | `false` | `enableAccessTokenForwarding` is a flag which tells whether the access_token should be forwarded as `Authorization` header or not |
| gloo.authConfig.spec.configs.authExtensionPlugin.config.enableJwtContentForwarding | bool | `false` | `enableJwtContentForwarding` is a flag which tells whether the user roles should be forwarded as `Jwt-Content` header or not |
...
| gloo.authConfig.spec.configs.authExtensionPlugin.config.enableSubjectForwarding | bool | `false` | `enableSubjectForwarding` is a flag which tells whether the subject (q-number) should be forwarded as `Auth-QNumber` header or not |
| gloo.authConfig.spec.configs.authExtensionPlugin.config.enableUserInfoForwarding | bool | `false` | `enableUserInfoForwarding` is a flag which tells whether the user roles should be forwarded as `User-Info` header or not |
